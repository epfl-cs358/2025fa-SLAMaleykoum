...
// ---------------------------------------------------------
//             RECTANGLE OBSTACLE (for raycast)
// ---------------------------------------------------------
...
int main()
{
    ...
    sf::RenderWindow window(
        ...
        "GLOBAL PLANNER + DYNAMIC MAP (SFML3)"
    );

    // --- ROBOT POSE ---
    Pose2D robot{0.f, 0.f, 0.f, 0};

    // --- GOAL ---
    MissionGoal goal;
    goal.target_pose.x = -3.0f;
    goal.target_pose.y =  3.0f;
    goal.target_pose.theta = 0.f;
    goal.type = EXPLORATION_NODE;
    ...
    std::vector<BoxObstacle> boxes = {
        ...
        BoxObstacle({-2.0f,1.0f},{1.0f,1.0f},{1.0f,1.05f},{-2.0f,1.05f}),
        BoxObstacle({-2.0f,1.0f},{-2.0f,-1.0f},{-1.95f,-1.0f},{-1.95f,1.0f})
    };

    const int   lidar_pts = 2000;
    ...
    while (window.isOpen())
    {
        ...
        for (int i = 0; i < path.current_length - 1; i++)
        {
            ...
            line[0].position = {
                (path.path[i].x / res + grid_x / 2.f) * cell,
                (-path.path[i].y / res + grid_y / 2.f) * cell
            };
            line[1].position = {
                (path.path[i+1].x / res + grid_x / 2.f) * cell,
                (-path.path[i+1].y / res + grid_y / 2.f) * cell
            };
