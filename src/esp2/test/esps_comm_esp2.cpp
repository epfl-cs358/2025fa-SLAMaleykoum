/**
 * @file test_common_esp2.cpp
 * @brief Test suite for validating ESP-to-ESP communication on ESP2.
 * It is designed to be launch at the same time as its twin on esp1 
 ...
 * verify the UART communication layer implemented by Esp_link. It receives
 * predefined Pose2D and GlobalPathMessage structures, transmitted by ESP1, sends back the equivalent
 * and reports correctness through MQTT.
 *
 * Main components:
 ...
 *
 * Control loop:
 *   - setup_esps_comm_esp2() initializes WiFi, MQTT, and the UART link
 *   - loop_esps_comm_esp2() polls for incoming bytes, executes tests, and logs results
 *
...
void test_pos() {
    ...
    if (esp_link.get_pos(out)) {
        ...
        mqtt_print(pos_to_cstr(out), mqtt_topic_esps2_pos);
        if (out == pos_eps1) {
